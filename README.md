...
- 📲 **PWA 支援** - 可安裝至手機主畫面，支援離線快取
- 🔄 **自動更新提示** - 偵測到新版本時會提示使用者更新，避免使用舊的資料結構
...
本專案已啟用 PWA (Progressive Web App)，使用者可以將網站安裝至手機主畫面，獲得接近原生 App 的體驗。

### 安裝方式

- **Android**: 開啟網站後，瀏覽器會自動彈出「新增至主畫面」提示
- **iOS Safari**: 點選分享按鈕 → 「加入主畫面」
- **桌面 Chrome**: 網址列右側會出現安裝圖示

### 版本更新

當系統偵測到新版本時，會彈出更新提示對話框。建議點選「立即更新」以確保使用最新功能及資料結構，避免相容性問題。

### 離線支援

PWA 會快取靜態資源 (JS、CSS、字體等)，即使短暫斷線仍可瀏覽已載入的頁面。但加分/扣分等操作仍需網路連線以同步至 Firestore。
...
- vite-plugin-pwa + Workbox - PWA 離線快取與更新提示
