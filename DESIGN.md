...
colors:
  garden-mint: "#6FD3A0"
  garden-mint-deep: "#2E9E6E"
  clear-sky: "#7FD1E8"
  clear-sky-deep: "#4FB0D1"
  achievement-apricot: "#F4A660"
  achievement-apricot-deep: "#D68540"
  sunlight: "#F8E999"
  reward-lavender: "#A88BFA"
  reward-lavender-deep: "#8B5CF6"
  feedback-red: "#FF8A80"
  cocoa-ink: "#6B5B49"
  garden-mist: "#E3F6ED"
  soft-beige: "#F1F0E0"
...
typography:
  display:
    fontFamily: "Zen Maru Gothic, Varela Round, ui-rounded, system-ui, sans-serif"
    fontSize: "2rem"
    fontWeight: 900
    lineHeight: 1.2
...
Sweet Home 是一座放在掌心裡的家庭花園：家長能在忙碌的日常中快速完成紀錄，孩子則能清楚看到努力如何累積成目標與獎勵。畫面以薄荷綠霧面背景、白色資訊表面、可可棕文字與圓潤字體形成溫暖基底，再用晴空藍、杏桃橘、日光黃與獎勵薰衣草區分功能。
...
**Display Font:** Zen Maru Gothic（後備 Varela Round、ui-rounded、system-ui）  
**Body Font:** Zen Maru Gothic（後備 Varela Round、ui-rounded、system-ui）  
**Label Font:** Zen Maru Gothic（後備 Varela Round、ui-rounded、system-ui）

**Character:** 單一圓體同時承載標題、操作與資料，保留親切感並降低孩子的閱讀負擔。層級依靠字重與固定尺寸，不使用誇張的流體大標題。

### Hierarchy

- **Display**（900、2rem、1.2）：只用於登入或主要入口標題。
- **Headline**（900、1.5rem、1.3）：頁面問候、成員名稱與主要數值標題。
- **Title**（700、1.25rem、1.4）：區塊標題、卡片標題與對話框標題。
- **Body**（400、1rem、1.5）：說明、紀錄與主要內容；連續文字寬度不得超過 70ch。
...
系統採柔和抬升：背景、內容表面與互動層以色調分層為主，陰影只輔助重要卡片、固定導覽與 Modal。現有 `soft-card` 是較擴散的環境陰影；新元件若已有實色邊界，就不得再疊加寬廣陰影。

### Shadow Vocabulary

- **Ambient Card** (`0 10px 30px -12px rgba(46, 158, 110, 0.28), 0 2px 6px -2px rgba(46, 158, 110, 0.12)`): 僅供主要積分或目標表面，且不得同時使用裝飾性 1px 邊框。
- **Control Press** (`0 4px 0 rgba(0, 0, 0, 0.15)`): 主要按鈕的按壓深度；active 狀態向下位移 4px 並移除陰影。
...
- **Shape:** 主要操作為完整膠囊；次要圖示按鈕使用柔和圓角（12–16px）。
- **Primary:** 嫩芽薄荷配白字，標準內距 12px × 24px，4px 深色底邊提供可按壓感。
- **Hover / Focus / Active:** hover 僅微幅提亮；focus-visible 使用清楚外框；active 向下 4px 並移除底邊。disabled 降低透明度且停止互動。
...
- **Internal Padding:** 手機 16–20px，較寬畫面 24–32px。

### Inputs / Fields

- **Style:** 白底、2px 低對比可可棕邊框、16px 圓角、16px 內距；文字至少 16px，避免手機瀏覽器自動縮放。
